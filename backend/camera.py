...
import threading
import datetime
from database import connect_to_database, insert_video_record
...
class Camera:
    ...
    def arm(self):
        ...
        self.armed = True
        print(f"Camera {self.name} armed.")

    def disarm(self):
        self.armed = False
        self.camera_thread = None
        print(f"Camera {self.name} disarmed.")
    ...
    def run(self):
        ...
        Camera.cap = cv.VideoCapture(self.url)
        ...
        while self.armed:
            ...
            # If no person is detected, stop recording
            else:
                non_detected_counter += 1  # increment the counter
                if non_detected_counter >= 10:
                    if self.out is not None:  # if VideoWriter is initialized, release it
                        self.out.release()
                        self.out = None  # set it back to None
                        thread = threading.Thread(target=send_notification_with_video, args=(current_recording_name,datetimeDetect, self.name))
                        thread.start()
                        current_recording_name = None
